...
export default function AlertMsg () {
    ...
    return(
        <>
        { alert.isVisible ?
            <div className={`fixed  top-5  z-1000 left-1/2 -translate-x-1/2 flex items-center justify-center w-max gap-1 sm:text-xl px-3 py-2 rounded-3xl sm:font-semibold bg-gray-950 ${alert.isSuccess ? 'text-green-500'  : 'text-red-500'}`}>
                {alert.isSuccess ? <Verified /> : <XCircle className="text-red-500 w-4 h-4 sm:w-6 sm:h-6 rounded-[50%]"/>}
