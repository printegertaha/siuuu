            ...
            <section className='flex gap-4 items-center justify-end   '>
                ...
                { userStatus === 'authenticated' 
                    ...
                    <div className='flex items-center gap-x-3'>
                        <Link prefetch={false} className='capitalize' href='/my-account'>{userInfo?.user?.name?.slice(0,5)}</Link>
                        <LogoutBtn textValue={<CiLogout className='text-blue-950' />}/>
                    </div>
                    :
                    <Link prefetch={false} href={pathname === '/login' ? '/register' : pathname === 'register' ? '/login': '/my-account'} className='hidden min-w-max  capitalize min-[350px]:block tracking-wide'>
