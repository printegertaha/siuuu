...
export default function MyAccount() {
  ...
  return (
    ...
    <div className="min-h-[calc(100vh-80px)] bg-[#fafafa] p-2 sm:p-6 md:p-12 font-sans text-gray-900">

        <div className="animate-in fade-in duration-300">
          <p className="text-[12px] sm:text-base text-gray-700 mb-6 leading-relaxed">
            Hello <span className="font-bold text-gray-900">{userInfo?.user?.name}</span> (not {userInfo?.user?.name} ?{" "}
            <LogoutBtn iconValue={false}/>
            )
          </p>
